...
%       h_front(R,1) = plot(f(:,1),mirror.*front_response_calibrated,'linewidth',2);
        msg_front = msg_front + mirror.*front_response_calibrated;
    else
%       h_front(R,1) = plot(f(:,1),mirror.*20*log10(abs(H_front)),'linewidth',2);
        msg_front = msg_front + mirror.*20*log10(abs(H_front));
    end
  else
    if plot_calibrated_response 
      front_response = 20*log10(abs(H_front));
...
%         h_rear(R,1) = plot(f_rear(:,1),mirror.*rear_response_calibrated,'linewidth',2);
        msg_rear = msg_rear + mirror.*rear_response_calibrated;
      else
%         h_rear(R,1) = plot(f_rear(:,1),mirror.*20*log10(abs(H_rear)),'linewidth',2);
        msg_rear = msg_rear + mirror.*20*log10(abs(H_rear));
      end
    else
      if plot_calibrated_response 
        rear_response = 20*log10(abs(H_rear));
...
%%%%%%%% ploy the average msg curve %%%%%%%%%%%
msg_front = msg_front./N;
plot(f, msg_front,'linewidth',2);
leg = legend('IG',[legend_labels_front{N,1}(1:9)]);
...
if type_dual
  ...
  leg = legend('IG',[legend_labels_rear{N,1}(1:9)]);
  set(leg, 'Interpreter', 'none');
  skip_rest = 0;
  for k = 1:1:N
